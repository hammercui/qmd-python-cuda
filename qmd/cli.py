...
def collection_remove(ctx_obj, name):
    ...
    ctx_obj.config.collections = [c for c in ctx_obj.config.collections if c.name != name]
...
def collection_rename(ctx_obj, old, new):
    ...
    try:
        ...
        if found:
            ...
            console.print(f"[yellow]Note: Collection '{old}' not found in config, but DB update attempted.[/yellow]")
...
def index(ctx_obj):
    ...
    for col in ctx_obj.config.collections:
        ...
        for rel_path, content, doc_hash, title in crawler.scan():
            ...
            ctx_obj.db.upsert_document(col.name, rel_path, doc_hash, title, content, context=context_text)
            count += 1
...
def get(ctx_obj, target, lines, line_numbers):
    ...
    console.print(f"[bold cyan]Path:[/bold cyan] qmd://{doc['collection']}/{doc['path']}")
...
def multi_get(ctx_obj, pattern, max_bytes, output_json, output_md, output_files):
    ...
    if total_size > max_bytes * 1024:
        console.print(f"[red]Error:[/red] Total size ({total_size/1024:.1f} KB) exceeds limit ({max_bytes} KB).")
        return
...
def status(ctx_obj):
    ...
    if total > 0:
        ...
        if missing > 0:
            if missing / total > 0.1:
                console.print(f"[bold yellow]Warning:[/bold yellow] {missing} documents need embedding. Run '[bold]qmd embed[/bold]' for better results.")
            else:
    ...
    if stats["last_index_update"]:
        ...
        try:
            ...
            if diff.days > 14:
                console.print(f"[bold yellow]Warning:[/bold yellow] Index is {diff.days} days old. Run '[bold]qmd update[/bold]' to refresh.")
        except:
            pass
...
def vsearch(ctx_obj, query, collection, limit):
    ...
    vsearch = VectorSearch()
    collections = [collection] if collection else [c.name for c in ctx_obj.config.collections]
    ...
    for r in all_results:
        table.add_row(f"{r.score:.4f}", r.metadata.get("title", "N/A"), r.collection, r.path)
...
def query(ctx_obj, query, collection, limit, rerank):
    ...
    hybrid = HybridSearcher(ctx_obj.db)
...
def context_list(ctx_obj, collection):
    ...
    for c in contexts:
        table.add_row(c["collection"], c["path"] or "(root)", c["context"][:50] + "..." if len(c["context"]) > 50 else c["context"])
...
def context_remove(ctx_obj, collection, path):
    ...
    console.print(f"[yellow]Removed context for[/yellow] {collection}:{path or '(root)'}")
...
def context_check(ctx_obj):
    ...
    with_ctx = set(c['collection'] for c in ctx_obj.db.list_path_contexts() if c['path'] == '' or c['path'] == '.')
    ...
    if missing_cols:
        ...
        for col in missing_cols:
            ...
            console.print(f"  Suggestion: [dim]qmd context add --collection {col} \"Description\"[/dim]")
        console.print()
    ...
    for col in all_cols:
        ...
        if missing_dirs:
            ...
            for d in missing_dirs:
                console.print(f"    {d}")
            console.print(f"  Suggestion: [dim]qmd context add --collection {col} --path <path> \"Description\"[/dim]")
