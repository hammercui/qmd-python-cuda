...
**原因**：
- 🎯 **专注目标**：节省VRAM（4GB vs 8GB+）
- ✅ **简化架构**：减少HTTP接口（19个 → 8个）
- ✅ **提高效率**：SQLite操作无需序列化
- ✅ **保持灵活**：CLI直接操作SQLite
...
| 命令 | 说明 | 实现方式 |
|------|------|---------|
| `collection add` | 添加集合 | 直接SQLite |
| `collection list` | 列出集合 | 直接SQLite |
| `collection remove` | 删除集合 | 直接SQLite |
| `collection rename` | 重命名集合 | 直接SQLite |
| `index` | 索引文档 | 直接SQLite |
| `update` | 更新集合 | 直接SQLite |
| `context add` | 添加上下文 | 直接SQLite |
| `context list` | 列出上下文 | 直接SQLite |
| `context rm` | 删除上下文 | 直接SQLite |
...
|------|------|---------|
| `get` | 获取文档 | 直接SQLite |
| `multi_get` | 批量获取 | 直接SQLite |
| `status` | 索引状态 | 直接SQLite |
